...
export const JobMap = component$((props: Props) => {
  ...
  const updateMarkers = $(() => {
    ...
    props.jobs.forEach((job) => {
      ...
      const typeBadge = job.type
        ? `<span style="background: #f3f4f6; color: #374151; font-size: 10px; padding: 2px 6px; border-radius: 99px; font-family: sans-serif;">${job.type}</span>`
        : "";
      const salaryHtml = job.salary
        ? `<div style="margin-top: 4px; font-size: 11px; color: #059669; font-weight: 500; font-family: sans-serif;">${job.salary}</div>`
        : "";
  ...
  return (
    <div
      ref={mapContainerRef}
      class="w-full h-[500px] rounded-xl shadow-lg border border-gray-200 dark:border-gray-700"
    />
  );
});
