...
export interface JobsState {
  ...
  getJobs: (page?: number, limit?: number, filters?: JobFilters) => JobListing[];
  getFilteredJobs: (userSkills?: string[], userAvailability?: string) => JobListing[];
  getComments: (jobId: string) => Comment[];
  getCompanyScore: (companyName: string) => number;
}
...
export const JobsProvider = component$(() => {
  ...
  const jobsState: JobsState = useStore<JobsState>({
    ...
    getFilteredJobs: (userSkills?: string[], userAvailability?: string) => {
      ...
      const scoredJobs = jobsState.jobs.map((job: JobListing) => {
        let score = 0;
        ...
        const daysSincePosted = Math.floor(
          (Date.now() - job.publishDate.getTime()) / (1000 * 60 * 60 * 24)
        );
